...
Time series forecasting models use historical data to forecast future values based on consistent patterns observed in the past. However, time series are sometimes interrupted by unusual events, that jeopardise the regularity of the time series pattern. Such interruptions may take a variety of forms including from a temporary policy change to a natural disaster; a few examples are provided below:

- Outbreaks, epidemics, and pandemics may all have a substantial influence on population behaviour, causing fluctuations and affecting time series data in a wide range of areas such as hospital admissions, pharmaceutical sales, transportation, retail sales, tourism, transportation, and traffic. 

- Supply chain interruptions, such as strikes, trade disputes, or shortages, may cause inconsistencies in data, including sales, inventory levels, production, and deliveries . Firms may need to shut down for repair, overhaul, or maintenance purposes. These events force production to halt, leaving a gap in the data.

- Equipment failures in transportation: Vehicle, aeroplane, or rail malfunctions or breakdowns may result in missing or incorrect data points in transportation-related datasets . 

- Cyberattacks: Cybersecurity breaches may jeopardise data integrity, resulting in incorrect readings in datasets including financial transactions, network traffic, and user activity. 


The occurrence of such events presents a tremendous challenge to time series forecasting methods, limiting their capacity to automatically and accurately capture systematic information and forecast future values. Therefore, practitioners face a critical question: how to effectively handle time series data impacted by such disruptive events?  Many forecasters have faced this issue recently with the COVID-19 pandemic, where historical patterns have been severely disrupted due to lockdowns and other restrictions. This study is motivated by numerous conversations with academics and practitioners, which demonstrate widespread concern about dealing with data affected by events like the COVID-19 pandemic.

Time series models usually assume that the data will evolve in the future in a similar way to how it has evolved in the past. But a big event (such as COVID-19) can result in a future that is different from the past, at least in the short term.
...
Stock out, maintenancem sensors
...
The forecasts shown in @fig-walkers-plot4 are obtained by replacing the observations during the lockdown periods with estimates based on an ARIMA model applied to the remaining data. Then a new ARIMA model is fitted to the whole data set, and used to produce forecasts. As a result, the "forecasts" appear to interpolate across the lockdown periods, reflecting neither the true lockdown pattern, or the pattern that might have occurred without the lockdowns.
...
Finally, the ensemble approach is useful when there is uncertainty about which approach to use, and will often result in greater accuracy due the power of averaging.
